...
function Report-Printer-Overview($ThePrinterIn){
...
Write-Output ""
Write-Output "Printer Name: $ThePrinterIn"

if($ThePrinter.PrinterStatus -eq 0){
    Write-Output "Queue Status: Ready"
}else{
    Write-Output "Queue Status: Offline"
}



$QueuedPrintJobs = (Get-Printer -ComputerName "$TheComputerName" -Name "$ThePrinterIn").JobCount
Write-Output "Jobs In Queue: $QueuedPrintJobs"
...
if(!($SharedPrinterListFileExists)){
    ...
    Write-Output "Error Shared Printer List File Does Not Exist"
    exit 1
}
...
while (($CUrrentLine = $StreamReader.ReadLine()) -ne $null)
{
    ...
    Report-Printer-Overview "$ThePrinterName"
    $line_number++
}
